# -*- coding: utf-8 -*-
"""
BMW i3 EV 주행거리 예측 대시보드 — F1 레드 에디션
실행: streamlit run app_red.py
"""
...
# ── 페이지 설정 + F1 CSS ─────────────────────────────────────────
st.set_page_config(page_title='BMW i3 · EV Dashboard (F1 Edition)', page_icon='🏎️', layout='wide')
...
<style>
 ...
 .bmw-title {{
   ...
   border-left:5px solid {F1_RED}; padding-left:16px; line-height:1.2;
 }}
 .bmw-sub {{
 ...
 .pred-label {{
   color:{F1_LIGHT}; font-size:.85rem; letter-spacing:2px;
 ...
 .stSlider [data-baseweb=slider] div[role=slider] {{ background:{F1_RED}; }}

 /* 버튼 */
 .stButton>button {{
   background:{F1_RED}; color:#fff; border:0; border-radius:8px;
   font-weight:600; padding:.55rem 1.4rem;
 }}
 .stButton>button:hover {{ background:{F1_DARK}; }}
 ...
 .sec-head span {{ color:{F1_RED}; margin-right:6px; }}
...
with st.sidebar:
    ...
    <div style="padding:16px 0 8px 0">
      <div style="font-size:1.3rem;font-weight:700;color:{TXT}">🏎️ BMW i3</div>
    ...
    with col_r:
        ...
        if 'Distance' in df.columns:
            ...
            hist.add_trace(go.Histogram(
                ...
                marker_color=F1_RED, opacity=0.85,
                name='주행거리',
            ))
...
# ════════════════════════════════════════════════════════════════
    ...
    with right:
        ...
        st.markdown(
            ...
            f'<div style="color:{F1_LIGHT};font-size:.8rem;letter-spacing:2px;'
        ...
        st.caption(f'● 빨간 바 = 평균 속도 · 주황 눈금 = 최고 속도 ({inputs.get("Velocity_max",0):.0f} km/h)')
...
# ════════════════════════════════════════════════════════════════
    ...
    with col_l:
        ...
        sc.add_trace(go.Scatter(
            ...
            marker={'color': F1_RED, 'size': 5, 'opacity': 0.65},
            name='예측 vs 실제',
        ))
    ...
    with col_r:
        ...
        res_hist.add_trace(go.Histogram(
            ...
            marker_color=F1_RED, opacity=0.8, name='잔차',
        ))
    ...
    with col_a:
        ...
        rv.add_trace(go.Scatter(
            ...
            marker={'color': F1_RED, 'size': 5, 'opacity': 0.6},
        ))
    ...
    with col_r:
        ...
        if 'Duration' in df.columns and 'Distance' in df.columns:
            ...
            sc3.add_trace(go.Scatter(
                ...
                mode='markers',
                marker={'color': F1_LIGHT, 'size': 5, 'opacity': 0.6},
            ))
            sc3.update_layout(
                height=320, paper_bgcolor='rgba(0,0,0,0)',
                plot_bgcolor='rgba(0,0,0,0)',
...
# ════════════════════════════════════════════════════════════════
    ...
    with col_l:
        ...
        colors_fi = [
            f'rgba(232,0,45,{0.45 + 0.55 * v / max_imp:.2f})'
    ...
    with col_r:
        ...
        pie_fig = go.Figure(go.Pie(
            ...
            hole=0.45,
            marker_colors=[F1_RED, F1_DARK, F1_LIGHT, '#a00020', '#cc3355', LINE],
